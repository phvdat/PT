...
const UploadAudioHappyBirthday = () => {
  const [loading, setLoading] = useState(false);
  const [fileList, setFileList] = useState<UploadFile[]>([]);

  const listRef = ref(storage, 'audio-happy-birthday');
  ...
  const handleUpload = async (file: File) => {
    ...
    const storageRef = ref(
      storage,
      `audio-happy-birthday/${new Date().getTime()}${file.name}`
    );
  ...
  const handleRemove = async (file: UploadFile) => {
    const storageRef = ref(storage, `audio-happy-birthday/${file.name}`);
...
export default UploadAudioHappyBirthday;
